...
<body>
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input id="newPassword" name="newPassword" type="password" value="" maxlength="50" minlength="6" class="${empty user.id?'required':''}" onkeyup="this.value=this.value.replace(/\W/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
				<c:if test="${empty user.id}"><span class="help-inline"><font color="red">*</font> </span></c:if>
				<c:choose>
					<c:when test="${not empty user.id}">
						<span class="help-inline">若不修改密码，请留空。若修改密码，不少于6个字符，必须由字母+数字组成</span>
					</c:when>
					<c:otherwise>
						<span class="help-inline">不少于6个字符，必须由字母+数字组成</span>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
