...
<%@ attribute name="vedioDomId" type="java.lang.String" required="false" description="视频DOM对应的ID"%>
...
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		clearIdInput();
		${input}Preview();
		//<c:if test="${type eq 'vedio'}">
			$.fn.loadVedio("#${vedioDomId}", url);
		//</c:if>
	...
	function ${input}Preview(){
		var prefix = "${fns:getConst('USERFILES_BASE_URL')}";
		var li, urls = $("#${input}").val().split("|");
		$("#${input}Preview").children().remove();
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){
				var src = urls[i];
				if(urls[i].indexOf(prefix) == -1){
					src = "${fns:getLiveEnv('webapp_domain')}" + "/upload/" + urls[i];
				}
				//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li><img src=\""+src+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
				li = "<li><a href=\""+src+"\" url=\""+urls[i]+"\" target=\"_blank\">"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</a>";//</c:if>
