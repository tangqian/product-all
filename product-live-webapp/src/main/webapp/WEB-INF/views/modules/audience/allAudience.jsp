...
<html>
	...
	<body>
		...
		<div class="fr w830">
			...
			<h1 class="page-title">全部观众<span class="page-title-min">（${fns:getRoomById(roomId).name}） </span></h1>
			<div class="mt20 set-service">
			<form:form id="allAudienceForm" modelAttribute="audience">
				<div class="clearfix mt10">
					<!-- 导出名单 -->
					<a href="/speaker/audience/export?expType=all&roomChat.roomId=${roomId }" class="button-a fl">导出名单</a>
					<div class="fr">
						<!-- 查询 -->
				...
				<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}" />
					<table width="100%" cellspacing="0" cellpadding="0" border="0" class="table data-table">
						<tr>
							<th width="210" class="align-center" style="border-left: 1px solid #dbdbdb;">公司</th>
							<th width="60" class="align-center" style="border-left: 1px solid #dbdbdb;">姓名</th>
							<th width="60" class="align-center" style="border-left: 1px solid #dbdbdb;">邮箱</th>
							...
							<th width="110" class="align-center" style="border-left: 1px solid #dbdbdb;">进入时间</th>
						</tr>
						...
						<tr>
                            ...
                            <td class="align-center">${item.user.email }</td>
                            <td class="align-center">${item.department }</td>
                            <td class="align-center">${item.job }</td>
                            ...
                            <td class="align-center">
                            	<fmt:formatDate value="${item.enterDate }" pattern="yyyy-MM-dd HH:mm" />
                            </td>
                        </tr>
						</c:forEach>
					</table>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$("#headCrumbs").append("<span class='em1'>></span>").append("<a href='${ctx}/speaker/audience/data'>观众数据</a>")
					.append("<span class='em1'>></span>").append("<span>全部观众</span>");
			});
			
			function page(n, s) {
				if(n) $("#pageNo").val(n);
				if(s) $("#pageSize").val(s);
				$("#allAudienceForm").attr("action", "${webSite }/speaker/audience/all?roomChat.roomId=${roomId }");
				$("#allAudienceForm").submit();
				return false;
			}
		</script>
	</body>
