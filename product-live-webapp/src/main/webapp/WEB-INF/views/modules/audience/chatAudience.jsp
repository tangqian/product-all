...
<html>
	...
	<body>
		...
		<div class="fr w830">
			...
			<h1 class="page-title">聊天观众<span class="page-title-min">（${fns:getRoomById(roomId).name}） </span>
				<a href="/room/chat/list?roomId=${roomId }" class="blue see-all-chat">查看全部聊天记录</a>
			</h1>
			<div class="mt20 set-service">
			<form:form id="chatAudienceForm" modelAttribute="audience">
				<div class="clearfix mt10">
					<!-- 导出名单 -->
					<a href="/speaker/audience/export?expType=chat&roomChat.roomId=${roomId }" class="button-a fl">导出名单</a>
					<div class="fr">
						<!-- 查询 -->
				...
				<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}" />
					<table width="100%" cellspacing="0" cellpadding="0" border="0" class="table data-table">
						<tr>
                            ...
                            <th width="" class="align-center" style="border-left: 1px solid #dbdbdb;">聊天记录</th>
						</tr>
						...
						<tr>
                            ...
                            <td class="align-center">
                                <div class="all-chat-input">
                                	<c:forEach items="${item.roomChat.chats}" var="chat">
	                               		<fmt:formatDate value="${chat.createDate }" pattern="yyyy-MM-dd HH:mm:ss" />
	                                	<br />${chat.content }<br />
                                	</c:forEach>
                                </div>
                            </td>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$("#headCrumbs").append("<span class='em1'>></span>").append("<a href='${ctx}/speaker/audience/data'>观众数据</a>")
					.append("<span class='em1'>></span>").append("<span>聊天观众</span>");
			});
			
			function page(n, s) {
				if(n) $("#pageNo").val(n);
				if(s) $("#pageSize").val(s);
				$("#chatAudienceForm").attr("action", "${webSite }/speaker/audience/chat?roomChat.roomId=${roomId }");
				$("#chatAudienceForm").submit();
				return false;
			}
		</script>
	</body>
